...
namespace Lab2_task2
{
    public partial class Form1 : Form
    {
        ...
        private string connectionString = @"server=192.168.247.144; port=3306; database=BOYCHUK_LAB2; user=root; password=PASSWORD;";
        ...
        // check that database has two expected records, if not we will add them 
        ...
        new string[] { "shampoo", "Washing the head" }
            ...
            foreach (var expectedType in expectedTypes)
            {
                ...
                // with this request we check that record exists (second table)
                ...
                using (var command = new MySqlCommand(request, connection))
                {
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        if (!reader.Read())
                        {
                            ...
                            using (var insertCommand = new MySqlCommand(insertRequest, connection))
                            {
                                insertCommand.Parameters.AddWithValue("@TypeName", typeName);
                                insertCommand.Parameters.AddWithValue("@Description", description);
                                // execute(run) query and returns number of affected rows
        ...
        private void executeSelect()
        {
            ...
            if (useId && useType)
            {
                ...
                if (int.TryParse(textBox1.Text, out var id))
                {
                    if (id < 1)
                    {
                        MessageBox.Show("Id can not be negative");
                        return;
                    }
                }
                if (comboBox1.Text == "")
        ...
        private void ExecuteAdd()
        {
            ...
            using (var command = new MySqlCommand("SELECT CosmeticsTypeID FROM CosmeticsTypes WHERE TypeName = @TypeName", connection))
            {
                command.Parameters.AddWithValue("@TypeName", typeName);
